...
export class Sources {
  ...
  public getTokenUrl(sourceEntity: SourcesEntity, contract?: string, tokenId?: string) {
    if (config.chainId == 1) {
      ...
      if (sourceEntity.metadata.tokenUrlXLayer && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlXLayer,
          "${contract}",
          contract
        );
        return _.replace(sourceEntity.metadata.url, "${tokenId}", tokenId);
      }
    } else if (config.chainId == 10241025) {
      if (sourceEntity.metadata.tokenUrlHalTestnet && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlHalTestnet,
          "${contract}",
          contract
        );
        return _.replace(sourceEntity.metadata.url, "${tokenId}", tokenId);
    ...
    } else if (config.chainId == 33139) {
      if (sourceEntity.metadata.tokenUrlApeChain && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlApeChain,
          "${contract}",
          contract
        );
        return _.replace(sourceEntity.metadata.url, "${tokenId}", tokenId);
    ...
    } else if (config.chainId == 3776) {
      if (sourceEntity.metadata.tokenUrlAstarZkevm && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlAstarZkevm,
          "${contract}",
          contract
        );
    ...
    } else if (config.chainId == 70800) {
      if (sourceEntity.metadata.tokenUrlApexTestnet && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlApexTestnet,
          "${contract}",
          contract
        );
      ...
      if (sourceEntity.metadata.tokenUrlBerachainTestnet && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlBerachainTestnet,
          "${contract}",
          contract
        );
    ...
    } else if (config.chainId == 17069) {
      if (sourceEntity.metadata.tokenUrlGarnet && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlGarnet,
          "${contract}",
          contract
        );
    ...
    } else if (config.chainId == 690) {
      if (sourceEntity.metadata.tokenUrlRedstone && contract && tokenId) {
        sourceEntity.metadata.url = _.replace(
          sourceEntity.metadata.tokenUrlRedstone,
          "${contract}",
          contract
        );
