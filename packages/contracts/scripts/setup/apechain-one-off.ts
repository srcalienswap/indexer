...
async function main() {
    ...
    fs.writeFileSync("deployed-contracts-apechain.txt", `${contractName}: ${reservoirAddress}\n`);
    await verify(reservoirAddress, args);

    const conduitControllerAddress = "0x00000000F9490004C11Cef243f5400493c00Ad63"; // ape chain
    ...
    fs.appendFileSync("deployed-contracts-apechain.txt", `${contractName}: ${reservoirApprovalProxyAddress}\n`);
    await verify(reservoirApprovalProxyAddress, args);

    const alienswapAddress = "0x790A46E50D154A8fE012E537cc1613dB80BbC9c3"; // apechain chain
    ...
    fs.appendFileSync("deployed-contracts-apechain.txt", `${contractName}: ${alienswapV2ModuleAddress}\n`);
    await verify(alienswapV2ModuleAddress, args);

    // open channel on conduit controller for ReservoirApprovalProxy
    const conduitAddress = "0x029EA8CC764F0cfA943D304F7539a04505DB2A61"; // mint chain
