...
async function main() {
    ...
    fs.writeFileSync("deployed-contracts-duckchain.txt", `${contractName}: ${reservoirAddress}\n`);
    await verify(reservoirAddress, args);

    const conduitControllerAddress = "0x00000000F9490004C11Cef243f5400493c00Ad63"; // duckchain
    ...
    fs.appendFileSync("deployed-contracts-duckchain.txt", `${contractName}: ${reservoirApprovalProxyAddress}\n`);
    await verify(reservoirApprovalProxyAddress, args);

    const alienswapAddress = "0xC4360Cec9D8C8EA254C24c904D8d6E48f5388f7e"; // duckchain
    ...
    fs.appendFileSync("deployed-contracts-duckchain.txt", `${contractName}: ${alienswapV2ModuleAddress}\n`);
    await verify(alienswapV2ModuleAddress, args);

    // open channel on conduit controller for ReservoirApprovalProxy
    const conduitAddress = "0x029EA8CC764F0cfA943D304F7539a04505DB2A61"; // duckchain
